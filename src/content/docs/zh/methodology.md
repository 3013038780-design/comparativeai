...
引用与免责另立独立页面：[引用与免责](/zh/methodology/citation-disclaimer/)。
...
- **Code of Practice**（如 [GPAI CoP](/zh/rules/eu/gpai-code-of-practice/)）：AI Act 第 56 条授权，签署 = 合规推定。是 **hEN 尚未到位前的过渡合规路径**
- **欧委会指引 / AI Office 指引**：填补条款模糊，提供执法预期（禁止清单指引、GPAI 指引等）
- **成员国 DPA / MSA 执法**：把硬法落地的执行机制（[France CNIL](/zh/subnational/eu/france-cnil-ai/)、[Spain AESIA](/zh/subnational/eu/spain-aesia/)、意大利 Garante、爱尔兰 DPC 等）
...
难以归类的条目，单独开一份记录：[法律位阶边界情况](/zh/methodology/citation-disclaimer/#hard-soft-edge-cases)。当前已知的边界情况包括：
...
- 标明对应条款号和原文段落（方便核对）
- 术语用法遵循本站术语对照表（见 [引用与免责](/zh/methodology/citation-disclaimer/)）
...
**版本表现在页面底部显示**（Starlight 的 `lastUpdated` 功能，基于 Git commit 时间）。重要修订同步记入 [更新日志 Updates](/zh/updates/)。
