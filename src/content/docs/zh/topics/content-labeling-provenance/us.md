---
title: 美国 — 生成内容标识
description: 美国对 AI 生成内容标识的联邦立法进展与州法差异
sidebar:
  order: 3
  label: 美国
---

## 规则层级

美国在该议题上呈现**州法先行、联邦在议、行业承诺补充**的格局：

- **联邦行政令**：[EO 14110](/zh/rules/us/eo-14110-biden-ai/)（2023-10，2025-01 已被撤销）曾要求商务部/NIST 研究水印与出处标准；
  继任行政令（[EO 14179](/zh/rules/us/eo-14179-removing-barriers-ai/)、
  [EO 14365](/zh/rules/us/eo-14365-national-ai-framework/)）方向转向"反过度监管 + 抢占州法"。
- **联邦立法（在审）**：
  - *COPIED Act*（出处来源、保护艺术家）
  - *NO FAKES Act*（非自愿深度伪造、人格权）
  - *DEFIANCE Act*（私密伪造影像的民事救济）
- **联邦平台移除法（已生效）**：技术中立的 **TAKE IT DOWN Act** 要求受覆盖平台为非自愿私密影像提供移除流程，包含 AI 生成伪造内容；FTC 于 2026 年 5 月 19 日开始执法，次日发出警告函。
- **州法**：加州、德州、华盛顿州、明尼苏达州等已通过与 AI 标识相关的专项法，主要聚焦 **选举** 和 **性暴力伪造** 场景。
- **行业承诺**：2023 年白宫牵头的主要 AI 公司自愿承诺（watermarking、provenance），以及 C2PA 联盟的技术规范。

## 义务主体（典型州法模式）

- **内容发布者**：在选举类合成内容上必须标注来源。
- **大型平台**：对可疑合成内容承担核验、标注、下架义务。
- **开发者**：目前多数州法不直接对基础模型开发者施加标识义务。

## 核心义务（共性）

1. **选举类**：投票前 N 天内传播未标注的合成内容构成违法（多数州为 60–90 天）。
2. **非自愿的性相关合成**：几乎所有州法都禁止并允许民事求偿，部分州有刑事处罚。
3. **一般消费者场景**：目前仍以行业自律为主，联邦无统一命令。

## 执法机构与处罚

- **FTC**：通过 "Section 5 — unfair or deceptive acts" 起诉欺骗性 AI 宣传。
- **各州检察长**：州法层面的首要执法者。
- **私人诉权**：部分州法（如 NO FAKES 类提案）赋予个人直接起诉权。

## 2026 年 5 月实践观察

TAKE IT DOWN Act 没有创设通用来源标识义务，其作用属于事后救济：未经同意的合成私密内容上线后，受覆盖平台必须提供可使用的通知和移除流程。因此应与水印、选举披露规则分开分析。

一手来源：[FTC 2026 年 5 月执法新闻稿](https://www.ftc.gov/news-events/news/press-releases/2026/05)。
